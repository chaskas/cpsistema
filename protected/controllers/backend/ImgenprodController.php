...
class ImgenprodController extends Controller
{
    /**
    ...
    {
        $model = Imgenprod::model()->findByPk($id);
        if ($model === null)
...
// $this->performAjaxValidation($model);
        ...
        if (isset($_POST['Imgenprod'])) {
            $model->attributes = $_POST['Imgenprod'];
            if ($model->save())
                $this->redirect(array('view', 'id' => $model->idImgP));
        }
...
// if AJAX request (triggered by deletion via admin grid view), we should not redirect the browser
    ...
    {
        $dataProvider = new CActiveDataProvider('Imgenprod');
    ...
    {
        $model = new Imgenprod('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['Imgenprod']))
            $model->attributes = $_GET['Imgenprod'];
    ...
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'imgenprod-form') {
    ...
    public function actionUpload()
    {
        ...
        $model = new Imgenprod('upload');
        $model->picture = CUploadedFile::getInstance($model, 'picture');
    /*    if ($model->picture !== null  && $model->validate(array('picture')))
        {
            $model->picture->saveAs(
                Yii::getPathOfAlias('frontend.www.files').'/upload/'.$model->picture->name);
            $model->Ruta = $model->picture->name;
            // save picture name
            if( $model->save())
            {
                ...
                $data[] = array(
                 ...
                 //   'thumbnail_url' => $model->getImageUrl(Imgenprod::IMG_THUMBNAIL),
                    // we need to include the action that is going to delete the picture
                    // if we want to after loading
                    'delete_url' => $this->createUrl('Imgenprod/delete',
