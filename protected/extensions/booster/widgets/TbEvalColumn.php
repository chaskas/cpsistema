 ...
 */
class TbEvalColumn extends TbDataColumn {
...
function() {
	...
	{
        Yii::app()->clientScript->registerScript('bulk','$("#input-id").rating();');
	...
	protected function renderDataCellContent($row, $data)
    {
        ...
        $eval = Evaluacionpr::model()->find('OrdenCompra_id=:ocid', array(':ocid' => $data->primaryKey));
        ...
        if (!is_null($eval)) {
            ...
            $this->grid->controller->widget('ext.DzRaty.DzRaty', array(
                'name' => 'Promedio',
                'value' => $eval->Promedio,
                'htmlOptions'=> array(
                    'data-placement' => 'left',
                    'data-toggle' => 'popover',
                    'data-content' => $eval->Comentario,
                    'data-title' => 'Promedio Evaluación: '.$eval->Promedio,
            ...
            If ($eval->Respuesta != '') {
                echo '<i class="fa fa-comments-o" title="" data-placement="bottom" data-toggle="popover"   data-title="Replica"  data-content="' . $eval->Respuesta . '"></i>';
                //  echo '<span class="fa fa-' . $button['icon'] . '"></span> <b>N°</b>';
            } else {
                $this->grid->controller->widget('TbEditable', array(
                    'type' => 'text',
                    'name' => 'Respuesta',
                    'pk' => $eval->idEvaluacionPr,
                    //   'text'      => 'John',
                    'url' => $this->url,
                    'title' => 'Ingrese sus descargos',
                    'placement' => 'left',
                    'emptytext' => '<i class="fa fa-reply" title="Replicar Evaluación"></i>',
                ));
