 ...
 * Modificación para usar fon awsome
 */
 ...
 */
class TbButtonAwesome extends TbWidget {
	...
	public function init() {
		...
		if ($this->hasDropdown()) {
			...
			$this->label .= ' <i class="fa fa-angle-down"></i>';
		...
		if (isset($this->icon)) { // no need for implode as newglyphicon only supports one icon
			if (strpos($this->icon, 'icon') === false && strpos($this->icon, 'fa') === false) {
				$this->icon = 'fa fa-' . $this->icon; // implode(' glyphicon-', explode(' ', $this->icon));
